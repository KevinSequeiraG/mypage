...
const PersonalProjectDeskModal = () => {
    ...
    return (
        <>{showModal ? <div className={`min-h-screen max-w-[100vh] lg:max-w-[100%] w-full bg-opacity-70 bg-black absolute top-0 z-[100]`}>
            ...
            <div className="text-center animate__animated animate__flipInX bg-gray-300 dark:bg-gray-800 w-[90%] lg:w-3/6 px-6 py-4 h-auto rounded-[1rem] top-1/4 left-[5%] lg:top-1/3 lg:left-1/4 -translate-x-1/2 -translate-y-1/2 absolute opacity-100 max-h-[50%] overflow-auto dark:text-gray-300">
                <h2 className="text-[3rem]">{t(window.localStorage.getItem("project-title"))}</h2>
                {
                    JSON.parse([window.localStorage.getItem("project-gallery")])?.map((image, i)=>{
                        return(<img className="max-w-[70%] mx-auto my-4" src={image} key={i}/>)
                    })
                }
